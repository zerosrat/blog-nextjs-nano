...
spoiler: 这是我在读研时做的项目，项目名为「高校科技服务平台」，平台主要为高校提供人才社交、项目发布与承接这两大功能。我主要负责项目的客户端开发与项目管理：Web 浏览器端（PC 端）和移动端（IOS + Android）。
--- */}
...
export const metadata = {
  ...
  date: '2018-11-13',
  alternates: {
    canonical: '/n/2018/summary-of-ustsp',
  },
}
...
这是我在读研时做的项目，项目名为「高校科技服务平台」，平台主要为高校提供人才社交、项目发布与承接这两大功能。我主要负责项目的前端开发，包括桌面浏览器端（PC 端）和移动端（IOS + Android），以及项目管理：。

## PC 端开发

### 技术栈

整体选择了 React 全家桶作为技术栈。视图层框架选择了 React，选择了 create-react-app 脚手架生成这个项目，路由框架选择了 React Router。
...
因为团队内没有 IOS or Android 开发者，都是前端开发者，选择了当前热门的 React Native 来进行跨平台开发。项目整体的技术栈也是 React 全家桶。项目是基于 create-react-native-app 这个脚手架生成的。
...
HTTP 库依旧是选择了 axios，和桌面端统一，当然 fetch 也不失为一种选择。
...
在进行开发之前，其实对 RN 开发坑多有所耳闻，自己真正参与到开发时确实遇到了不少问题。好在大多数问题通过 Stack Overflow 或是 Github issues 就可以解决。对于没有原生开发的小团队而言 All in RN 或许是个不错的选择。之前 Airbnb 团队发布了弃用 RN 的博客，以及 Dropbox 团队放弃了用 C++ 来进行跨平台开发，还是引起了我们的思考。同时精通三端的开发者以及精通 C++ 的前端开发者都太少了，跨平台开发还是存在这一些不可忽视的问题。再看看最近很热的 Flutter，它和 RN 的渲染在不同层级，前者是画布后者是原生控件。未来跨平台开发会如何呢，让我们拭目以待。

遇到过一个比较糟心的问题是： [IOS 平台下 controlled TextInput 组件的中文输入问题](https://github.com/facebook/react-native/pull/18456)，属于 RN 框架本身的 bug，直到 v0.57 正式发布之前，要根据官方的源码修改文档修改 TextInput 组件代码来修复这个bug。

## 不足之处

两个项目都缺少测试代码的编写，在项目的预上线期间发现了不少 bug，我也在思考编写测试代码的成本是否小于修改 bug 的成本，写测试对项目健壮性的帮助不言而喻。在项目时间充裕的情况下，可以推动团队成员一起给基础组件来写单元测试，这对团队本身也是一种成长。
...
这个项目现在已经告一段落，进入维护阶段了。通过 learn by doing 收获了挺多，对 react 技术栈、前端工程化和项目管理也有了实战上的理解。也感受到在做一个项目的过程中，只是复制粘贴，当一个 API caller，没有解决并记录自己遇到的问题，更别说站在高点思考整个架构，这么做完一个项目也只是原地踏步罢了。所以要不仅仅是写业务，还要记录痛点难点并思考为什么。
