...
由于调用 C++ 后数据不是同步返回的，所以得来看看**异步回调**是怎么实现的，代码如下。核心还是通过 JSC 的 API 获取到 JS 环境上全局变量上的回调方法，即 `global.__fbBatchedBridge.invokeCallbackAndReturnFlushedQueue`，然后组装参数：把 C++ 类型的参数转为 JS 类型的，最后调用并传参。
...
![async-call-flow](/static/images/mini-rn-1/async-call-native.png)

## 彩蛋

经过了一路旅途，很高兴你可以看到最后！🎉 文章篇幅有限，代码也只是贴了相关片段，如果想要了解完整的实现，可以详参项目源码。

**项目地址**: [https://github.com/zerosrat/mini-react-native](https://github.com/zerosrat/mini-react-native)
...
3. **渲染系统**: 当前只支持逻辑通信，后续将接入渲染层，能够渲染简单的组件
6. **新架构探索**: 当前只支持桥架构，后续将研究 JSI、TurboModules 等新架构特性
